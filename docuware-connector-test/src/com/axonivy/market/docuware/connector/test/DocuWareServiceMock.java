...
public class DocuWareServiceMock {
	...
	public Response getDocument(@Context HttpServletRequest req,  @PathParam(value="FileCabinetId") String fileCabinetId, @PathParam(value="DocumentId") String documentId ) {
		...
		if(!isAuthenticated(req)) {
			...
			return Response.ok(load("xml/document.xml"))
			        .type(MediaType.APPLICATION_XML).build();
		}
	}
	...
	public Response updateDocument(@Context HttpServletRequest req,  @PathParam(value="fileCabinetId") String fileCabinetId,   @PathParam(value="documentId") String documentId) {
		if(!isAuthenticated(req)) {
			// note: the real service would send details
			return Response.status(401).build();
		}
		else {
			return Response.ok(load("xml/documentIndexFields.xml"))
			        .type(MediaType.APPLICATION_XML).build();
	...
	@Produces(MediaType.APPLICATION_XML)
	@Path("FileCabinets/{FileCabinetId}/Documents/{DocumentId}")
	public Response deleteDocument(@Context HttpServletRequest req,  @PathParam(value="FileCabinetId") String fileCabinetId,   @PathParam(value="DocumentId") String documentId) {
		if(!isAuthenticated(req)) {
			// note: the real service would send details
			return Response.status(401).build();
		}
			...
			if(documentId.equals(Constants.DOCUMENT_ID_ERRROR_CASE)) {
				...
				return Response.status(403).entity("xml/error.xml")
			        .type(MediaType.APPLICATION_XML).build();
			}else {
