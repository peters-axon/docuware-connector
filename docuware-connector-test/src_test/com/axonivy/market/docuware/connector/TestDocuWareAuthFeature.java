...
public class TestDocuWareAuthFeature {
  ...
  void noHost() {
    ...
    Assertions
            .assertThatThrownBy(() -> ClientBuilder.newClient().register(ft)
  ...
  void noLogonUrl_resolveAuto() {
    ...
    try {
      ClientBuilder.newClient().register(ft).property(Property.USERNAME, "Reguel")
              .property(Property.PASSWORD, "notMySecret").property(Property.HOSTID, "axoooooniiiivy")
              .property(RestClientFactoryConstants.PROPERTY_CLIENT_ID, DW_REST_CLIENT_ID)
              .property("jersey.config.client.connectTimeout", 1).target("http://0.0.0.0/someResource")
              .request()
              .get();
    } catch (Exception ex) {
      // expected
    } finally {
      assertThat(logon.get()).isEqualTo("https://0.0.0.0/docuware/platform/Account/Logon");
    }
  }
}
