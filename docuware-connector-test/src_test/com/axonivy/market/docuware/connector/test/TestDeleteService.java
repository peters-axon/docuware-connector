...
public class TestDeleteService extends TestDocuWareConnector {
	...
	private static final BpmElement testeeDelete_2 = BpmProcess.path("DeleteService").elementName("deleteDocument(String, DocuWareEndpointConfiguration)");
	...
	public void deleteDocument(BpmClient bpmClient, ISession session, AppFixture fixture, IApplication app)  throws IOException {
		...
		ExecutionResult result = bpmClient.start().subProcess(testeeDelete).withParam("documentId", String.valueOf(DOCUMENT_ID)).execute();
	...
	public void deleteDocumentError(BpmClient bpmClient, ISession session, AppFixture fixture, IApplication app)  throws IOException {
		prepareRestClient(app, fixture);		
		ExecutionResult result = bpmClient.start().subProcess(testeeDelete).withParam("documentId", Constants.DOCUMENT_ID_ERRROR_CASE).execute();
		DeleteServiceData data = result.data().last();
	...
	@Test
	public void deleteDocumentWithEndpointConfiguration(BpmClient bpmClient, ISession session, AppFixture fixture, IApplication app)  throws IOException {
		prepareRestClient(app, fixture);
		DocuWareEndpointConfiguration configuration = prepareDocuWareEndpointConfiguration();
		ExecutionResult result = bpmClient.start().subProcess(testeeDelete_2).withParam("documentId", String.valueOf(DOCUMENT_ID)).withParam("configuration", configuration).execute();
		DeleteServiceData data = result.data().last();
		assertThat(data.getError()).isNull();
	}
